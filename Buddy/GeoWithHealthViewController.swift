//
//  GeoWithHealthViewController.swift
//  Buddy
//
    ...
    */
    ...
    func mapView(_ mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if control == view.leftCalloutAccessoryView {
            performSegue(withIdentifier: "buddiesWithHealthGoingSegue", sender: view)
        }
    }
    
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "buddiesWithHealthGoingSegue" )
        {
            print("**********Segue1*********")
            let next = segue.destination as! UINavigationController
            let nextController = next.topViewController as! BuddiesWithHealthGoingTableViewController
